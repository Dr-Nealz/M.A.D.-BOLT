...
- **NVIDIA Provider Integration** - Direct NVIDIA LLM model access
- **9router Provider Integration** - Unified routing across multiple providers

## Features

- **AI-powered full-stack web development** for **NodeJS based applications** directly in your browser.
- **Support for 21+ LLMs** with an extensible architecture to integrate additional models (including NVIDIA and 9router).
...
## Adding New LLMs (NVIDIA and 9router)

bolt.diy supports a modular architecture for adding new LLM providers. NVIDIA and 9router providers are now included out-of-the-box:

### NVIDIA Provider

The NVIDIA provider gives you access to NVIDIA's latest LLM models, including:

- **Claude Nemotron series** - NVIDIA's best-in-class reasoning models
- **Llama 3.1 Nemotron** - Open source models fine-tuned by NVIDIA
- **Mistral** models through NVIDIA's infrastructure
...
- The system will auto-enable the NVIDIA provider
- Models will be automatically discovered from NVIDIA's API

#### NVIDIA Models:
The NVIDIA provider supports models like:
- `nvidia/llama-3.1-nemotron-70b-instruct` - 128K context, reasoning optimized
- `nvidia/llama-3.1-nemotron-8b-instruct` - Lightweight, fast inference
- `nvidia/mistral-7b-instruct-v0.1` - Proven open-source model

### 9router Provider

The 9router provider gives you unified access to multiple LLM providers through a single interface:

#### 9router Configuration Options:

**Local 9router Instance:**
```bash
# For local 9router development
NINEROUTER_API_KEY=your_api_key_here
NINEROUTER_BASE_URL=http://localhost:3000
```

**Hosted 9router:**
```bash
# For a hosted 9router instance
NINEROUTER_API_KEY=your_api_key_here  
NINEROUTER_BASE_URL=https://your-9router-instance.com/api
```

#### 9router Features:
- **Unified Routing** - Route between different providers based on needs
- **Model Discovery** - Automatic discovery of available models across providers
- **Load Balancing** - Intelligent distribution across provider instances
- **Fallback Mechanisms** - Seamless switching between providers

#### Supported Model Categories via 9router:
- **NVIDIA Models** - Llama Nemotron series
...
- **Together AI** - Meta models and more

#### Configuration Examples:

**For local development:**
```bash
# 9router setup for local development
...
**For production:**
```bash
# 9router setup for production
NINEROUTER_API_KEY=your_production_key_here
NINEROUTER_BASE_URL=https://router.yourdomain.com/api
```
...
1. **Choose specific providers** based on your needs
2. **Use 9router for unified access** across multiple providers
3. **Fallback between providers** based on availability and cost
4. **Configure in Settings** → **Providers** section

### Quick Start with NVIDIA and 9router
...
3. **Configure providers:**
   ...
   - Enable NVIDIA and/or 9router providers
   - Set your preferred model for each provider

4. **Start using:** Begin your conversation with bolt.diy!

### Provider Comparison

| Feature | NVIDIA | 9router | 9router Advantage |
|---------|--------|---------|------------------|
| **Model Selection** | 3 core models | 10+ model categories | Most comprehensive |
| **Setup Complexity** | Simple direct | Single interface | Easier management |
| **Fallback Options** | None | Multiple providers | Higher reliability |
| **Best For** | NVIDIA-specific models | Unified provider management | Enterprise routing |

### Troubleshooting

#### NVIDIA Provider Issues:
```bash
...
curl -H "Authorization: Bearer $NINEROUTER_API_KEY" \
     http://localhost:3000/v1/models
```

#### Common Solutions:
- **Both providers not showing**: Restart bolt.diy development server
- **API key rejected**: Verify key format and permissions
- **Models not loading**: Check environment variable spelling
- **Performance issues**: Try switching providers or models

### Additional Provider Integrations

bolt.diy also supports these providers:
...
After setting up NVIDIA and 9router providers:

1. **Configure API keys** in your `.env.local` file
2. **Restart bolt.diy** to auto-enable providers
3. **Navigate to Settings** → **Providers** to verify configuration
4. **Start using** your preferred provider for AI-powered development

The NVIDIA and 9router integrations make bolt.diy more powerful by providing access to NVIDIA's latest models and unified routing across multiple providers, giving you maximum flexibility and reliability for your AI development needs.
