...
function formatIssueBody(data: z.infer<typeof bugReportSchema>): string {
  ...
  if (data.includeEnvironmentInfo && data.environmentInfo) {
    ...
    if (data.environmentInfo.boltVersion) {
      body += `- M.A.D. BOLT-REMIX: ${data.environmentInfo.boltVersion}\n`;
    }
  ...
  body += '---\n*Submitted via M.A.D. BOLT-REMIX bug report feature*';
...
export async function action({ request, context }: ActionFunctionArgs) {
  ...
  try {
    ...
    const targetRepo =
      (context?.cloudflare?.env as any)?.BUG_REPORT_REPO || process.env.BUG_REPORT_REPO || 'Dr-Nealz/M.A.D.-BOLT';
    ...
    const octokit = new Octokit({
      auth: githubToken,
      userAgent: 'mad-bolt-remix-bug-reporter',
    });
